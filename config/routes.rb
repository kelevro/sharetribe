...
Kassi::Application.routes.draw do
  ...
  get '/:locale/allstrains' => 'homepage#index', as: :search_with_locale, constraints: ->(request) {
    locale_matcher_anchored.match(request.params["locale"]) &&
      CustomLandingPage::LandingPageStore.enabled?(request.env[:current_marketplace]&.id)
  }
  get '/allstrains' => 'homepage#index', as: :search_without_locale, constraints: ->(request) {
    CustomLandingPage::LandingPageStore.enabled?(request.env[:current_marketplace]&.id)
  }
  ...
  get '/:locale/allstrains', to: redirect('/%{locale}', status: 307), constraints: { locale: locale_matcher }
  get '/allstrains', to: redirect('/', status: 307)
